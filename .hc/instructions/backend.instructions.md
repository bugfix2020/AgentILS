...
- Keep conversation state derived from `src/store/conversation-store.ts` as the single truth source.
- Keep request-scoped interaction behavior anchored in `src/gateway/context.ts`.
