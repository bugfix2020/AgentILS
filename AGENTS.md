...
This repository is a TypeScript runtime project.  
本仓库是一个 TypeScript runtime 项目。
...
- `test/gateway/request-context.test.ts`
...
- task start: read `src/gateway/tools.ts` -> `src/orchestrator/conversation-orchestrator.ts` -> `src/store/memory-store.ts`  
  task start：先读 `src/gateway/tools.ts` -> `src/orchestrator/conversation-orchestrator.ts` -> `src/store/memory-store.ts`
- approval or feedback: read `src/gateway/tools.ts` -> `src/gateway/context.ts` -> `src/orchestrator/control-mode-orchestrator.ts`  
  approval 或 feedback：先读 `src/gateway/tools.ts` -> `src/gateway/context.ts` -> `src/orchestrator/control-mode-orchestrator.ts`
- verify or summary: read `src/orchestrator/verification-orchestrator.ts` -> `src/store/summary-store.ts`  
  verify 或 summary：先读 `src/orchestrator/verification-orchestrator.ts` -> `src/store/summary-store.ts`
- conversation state: read `src/store/conversation-store.ts` first  
  conversation state：优先先读 `src/store/conversation-store.ts`
...
- For conversation state, trust `src/store/conversation-store.ts` before other projections.  
  对于 conversation state，优先以 `src/store/conversation-store.ts` 为准，而不是其他投影层。
- For request-scoped interaction, trust `src/gateway/context.ts` and related gateway tests.  
  对于 request-scoped interaction，优先以 `src/gateway/context.ts` 和相关 gateway 测试为准。
