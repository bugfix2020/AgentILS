...
| `mcp/orchestrator/orchestrator.ts` | parked promise map（`Map<requestId, {resolve, reject}>`）+ subscribers Set + version；`park` / `resolve` / `cancel` / `pending` / `snapshot` / `sweepExpired`                                                                 |
