...
export function loadVsCodePromptPack(): PromptPackFile[] {
  ...
  return [
    ...
    // PR-E: stage envelope 字段契约（让 LLM 按 5 节点结构化填 reply）
    {
      ...
      content: readNormalized(
        join(templatesRoot, 'instructions', 'stage-envelope-contract.md'),
      ),
    },
  ]
}
...
export function installVsCodeAssets(options: {
  ...
  if (scope === 'workspace' || scope === 'both') {
    ...
    // started by the AgentILS VS Code extension (or `pnpm --filter @agentils/mcp start`).
      ...
      const destination = file.relativePath.includes('/agents/')
        ...
        : file.relativePath.includes('/instructions/')
          ? join(
              ...
              '.copilot',
              'instructions',
              'agentils-stage-envelope-contract.instructions.md',
            )
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const changes =
    options.command === 'install'
      ? installVsCodeAssets(options)
      : uninstallVsCodeAssets(options)
