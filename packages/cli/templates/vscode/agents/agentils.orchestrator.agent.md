...
- In VS Code, prefer `@agentils` when the goal is a long-running AgentILS session with WebView-owned interaction.
...
- Once the AgentILS WebView session is running, keep the main interaction inside the WebView and do not treat plain Copilot chat as the primary transcript surface.
