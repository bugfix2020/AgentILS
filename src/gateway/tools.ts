...
function registerTaskLifecycleTools(runtime: AgentGateServerRuntime) {
  ...
  server.registerTool(
    'conversation_get',
    {
      description: 'Read the current conversation state, including the active task snapshot and summary.',
      inputSchema: {
        runId: z.string().optional(),
      },
    },
  ...
  server.registerTool(
    'control_mode_get',
    {
      description: 'Read the current control mode, override state, and next action for a run.',
      inputSchema: {
        runId: z.string().optional(),
      },
    },
  ...
  server.registerTool(
    'task_summary_get',
    {
      description: 'Read the task summary document or a synthesized summary view for a run or task.',
      inputSchema: {
        runId: z.string().optional(),
